...
import {
  Container, Jumbotron, Button, Card, CardImg, CardText, CardBlock,
  CardTitle, CardSubtitle, Row, Col, Modal, ModalHeader, ModalBody,
  ModalFooter, Table, Alert
} from 'reactstrap';
...
export default class Content extends Component {
  ...
  deleteCartItem = (index) => {
    ...
    // using splice to delete one item
  ...
  render() {
    // 此處並未異動到state的
    const totalPrice = this.state.carts.reduce((acc, item) => acc + item.price, 0);
    return (
      <Container>
        {this.state.test}
          ...
          <Row>
            <Col sm={12} md={12}>
              <Jumbotron>
                ...
                <hr className="my-2" />
                <p className="lead">
                  <Button color="primary" onClick={this.toggle}>購物車({this.state.carts.length})</Button>
                </p>
              </Jumbotron>
            </Col>
          </Row>
                ...
                <Card>
                  ...
                  <CardBlock>
                    ...
                    <Button
                      ...
                      disabled={
                        // 購物車找尋該商品是否已存在
                        this.state.carts.find(item => item.id === product.id)}
                    >
                      Buy
                      </Button>
                  </CardBlock>
